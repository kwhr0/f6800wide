...
wire [13:0] iadr = pc[15:2];
wire [13:0] iadr1 = pc[1] ? iadr + 1'b1 : iadr;
wire [31:0] insn;

wire [13:0] dadr = adr[15:2];
wire [13:0] dadr0 = &adr[1:0] ? dadr + 1'b1 : dadr;
...
f6800wide f6800wide(.clk(clk), .reset(~rst_n), .wr_l(wr_l), .wr_u(wr_u),
	.pc_out(pc), .insn_in(insn),
	.adr_out(adr), .data_in(data_in), .data_out(data_out),
	.intreq(irq), .intack(iack), .nmireq(1'b0), .nmiack());
